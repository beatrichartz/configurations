module Configurations
  ...
  module Configurable
    ...
    module ClassMethods
      ...
      #     foo + bar + arg
      #   end
      # @example Define a configuration method on a nested property (it does only have access to sibling properties)
      ...
      # @param [Array, Symbol, Hash] properties the properties to install the callback on. If omitted, the callback will be installed on all properties that have no specific callbacks
      # @param [Proc] block the block to evaluate when a property has not been configured
      # @yield [Symbol] the property that has not been configured
      # @example Define a specific not_configured callback for some properties including a nested property
      #   not_configured :property1, :property2, property3: :property4 do |property|
      ...
      # @param [Proc] block the block to point the properties to when not configured
      #
      def nested_merge_not_configured_hash(*properties, &block)
        ...
        @not_configured.merge! zip_to_hash(block, *properties), &method(:configuration_deep_merge)
      end
      ...
      #
      def configuration_deep_merge(key, oldval, newval)
